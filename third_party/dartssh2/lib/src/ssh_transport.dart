...
/// KAMMEL patch: same idea as [SSHHostkeyVerifyHandler], but receives the raw
/// host key blob instead of an MD5 digest, so the caller can pin on a modern
/// hash (OpenSSH-style `SHA256:<base64>`). Upstream only exposes MD5, which is
/// too weak to base host-key verification on. Takes precedence over
/// [SSHHostkeyVerifyHandler] when both are set.
typedef SSHHostkeyBlobVerifyHandler = FutureOr<bool> Function(
  String type,
  Uint8List hostkeyBlob,
);
...
class SSHTransport {
  ...
  /// KAMMEL patch: see [SSHHostkeyBlobVerifyHandler].
  final SSHHostkeyBlobVerifyHandler? onVerifyHostkeyBlob;
  ...
  void _handleMessageKexReply(Uint8List payload) {
    ...
    // KAMMEL patch: prefer the blob handler so the app can pin on SHA256.
    final userVerified = onVerifyHostkeyBlob != null
        ? onVerifyHostkeyBlob!(_hostkeyType!.name, hostkey)
