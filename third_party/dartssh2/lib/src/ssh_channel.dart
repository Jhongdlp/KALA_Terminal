...
class SSHChannelController {
  ...
  void _sendWindowAdjustIfNeeded() {
    ...
    // KAMMEL patch (upstream bug, still present in 2.22.5): the original guard
    // was `if (_localWindow <= 0) return;`, which bails out precisely when the
    // receive window is exhausted — exactly when it must be replenished. If the
    // consumer pauses (any `pipe`/`addStream`, or our own backpressure in
    // services/tunnel_manager.dart) while the window drains to zero, the
    // channel never sends another WINDOW_ADJUST and the transfer deadlocks
    // forever. Reproducible as a hang at ~6 MB through a `-L` tunnel.
